...
test("evaluates method, status and attempt gates", () => {
  ...
  assert.deepEqual(evaluateRetry({ method: "GET", status: 404, attempt: 3, maxAttempts: 3 }, policy), { retry: false, delayMs: 0, reason: "status" });
...
test("computes retry delay without mutating inputs", () => {
  ...
  assert.deepEqual(evaluateRetry({ ...input, retryAfterMs: 250 }, policy), { retry: true, delayMs: 250, reason: "retry" });
  assert.deepEqual(evaluateRetry({ ...input, retryAfterMs: 0 }, policy), { retry: true, delayMs: 0, reason: "retry" });
...
test("rejects malformed retry inputs", () => {
  ...
  for (const input of [null, {}, { ...goodInput, method: "" }, { ...goodInput, method: 1 }, { ...goodInput, status: 503.5 }, { ...goodInput, status: NaN }, { ...goodInput, attempt: -1 }, { ...goodInput, attempt: 0.5 }, { ...goodInput, maxAttempts: -1 }, { ...goodInput, maxAttempts: 0.5 }, { ...goodInput, retryAfterMs: -1 }, { ...goodInput, retryAfterMs: Infinity }]) {
    assert.throws(() => evaluateRetry(input, goodPolicy), TypeError);
  }
  for (const policy of [null, {}, { ...goodPolicy, baseDelayMs: -1 }, { ...goodPolicy, baseDelayMs: NaN }, { ...goodPolicy, maxDelayMs: Infinity }, { ...goodPolicy, allowPost: "yes" }]) {
